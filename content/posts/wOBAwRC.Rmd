...
get_run_values <- function(plays, RE.matrix){
  ...
  # calculate average OBP wOBA across league

  OBP.league <- (sum(plays$H_FL > 0) + sum(plays$RESULT %in% c("Walk", "Hit by pitch")))/
  ...
  return(list("RV" = run.values, "OBP.league" = OBP.league, "wOBA.scale" = wOBA.scale, "RpPA" = runs.per.PA))
}
